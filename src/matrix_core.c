...
/* Возвращает размер элемента для конкретного MatType либо 0, если тип неизвестен */
static size_t element_size_of(MatType mat_type) {
  switch (mat_type) {
    case MAT_F64:  return sizeof(double);
    case MAT_INT32:return sizeof(int);
    default:       return 0;
  }
}
...
Matrix *mat_new(MatType mat_type, int rows, int cols) {
  ...
  if (!matrix) { free(block_pointer); return NULL; }
  ...
  matrix->row.vp   = (void**)block_pointer;
  matrix->flat_data = (unsigned char*)block_pointer + rowptrs_size_bytes;

  // проставим row-указатели
  if (mat_type == MAT_F64) {
    double *flat_data_as_double = (double*)matrix->flat_data;
    for (int row = 0; row < rows; ++row)
      matrix->row.d[row] = flat_data_as_double + (size_t)row * cols;
  } else if (mat_type == MAT_INT32) {
    int *flat_data_as_int = (int*)matrix->flat_data;
    for (int row = 0; row < rows; ++row)
      matrix->row.i[row] = flat_data_as_int + (size_t)row * cols;
  }
  return matrix;
}

/* Освобождает Matrix и, если нужно, её буфер */
...
/* Глубокое копирование матрицы в новую структуру */
Matrix *mat_clone(const Matrix *source) {
  ...
  memcpy(dest->flat_data, source->flat_data,
         (size_t)source->rows * (size_t)source->cols * element_size);
  return dest;
}
...
/* Оборачивает внешний буфер в Matrix, по желанию создавая массив указателей на строки */
Matrix *mat_wrap_buffer(MatType mat_type, void *flat_data_row_major,
  ...
  matrix->type  = mat_type;
  matrix->owns  = 0;
  matrix->block = NULL;                 // чужой буфер — не освобождаем
  matrix->flat_data = flat_data_row_major;
  matrix->row.vp    = NULL;

  // если нужно, сгенерим массив указателей на строки во «временном» блоке
  if (make_row_ptrs) {
    void **row_pointer_array = (void**)malloc((size_t)rows * sizeof(void*));
    if (!row_pointer_array) { free(matrix); return NULL; }
    matrix->row.vp = row_pointer_array;

    if (mat_type == MAT_F64) {
      double *flat_as_double = (double*)matrix->flat_data;
      for (int row = 0; row < rows; ++row)
        matrix->row.d[row] = flat_as_double + (size_t)row * cols;
    } else { // MAT_INT32
      int *flat_as_int = (int*)matrix->flat_data;
      for (int row = 0; row < rows; ++row)
        matrix->row.i[row] = flat_as_int + (size_t)row * cols;
    }
  }
  return matrix;
}
...
double **mat_rows_f64(Matrix *matrix) {
  ...
  if (!matrix->row.vp) {
    ...
    void **row_pointer_array = (void**)malloc((size_t)matrix->rows * sizeof(void*));
    if (!row_pointer_array) return NULL;
    matrix->row.vp = row_pointer_array;
